...
describe('Phase 15L.1 Ajukan Dokumen parity source guard', () => {
  ...
  it('keeps Step 2 compact and restores a compact grouped Step 3 with a page-local warm palette', () => {
    ...
    expect(reviewSource).toContain('bg-warning-surface')
    ...
    expect(source).toContain('bg-bg-surface')
    expect(source).toContain('from-brand-solid to-brand-gradient-to')
    expect(source).toContain('bg-brand-solid')
    ...
    expect(source).toContain('border-brand-border-strong bg-bg-surface')
  ...
  it('mirrors the Revisi-style pending upload row and Ajukan unsaved guard without backend changes', () => {
    ...
    expect(uploadButtonSource).toContain('border border-amber-200 bg-warning-surface')
