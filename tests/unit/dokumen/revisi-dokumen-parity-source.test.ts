...
describe('Phase 15L.2 Revisi Dokumen parity source guard', () => {
  ...
  it('keeps compact Revisi attachment polish for reset and replace actions', () => {
    ...
    expect(editorSource).toContain('function UploadValidationInline')
    expect(editorSource).toContain('getDocumentUploadValidationUiMessage')
    expect(editorSource).toContain('border border-amber-200 bg-warning-surface')
    ...
    expect(editorSource).toContain('border border-dashed border-brand-solid')
    expect(editorSource).toContain('rounded-xl border border-brand-border-strong bg-bg-surface')
    expect(editorSource).not.toContain('text-xs text-error">\\n                        <AlertCircle size={12} />\\n                        {uploadError}')
