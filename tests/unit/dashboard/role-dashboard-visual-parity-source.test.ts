...
describe('role dashboard visual parity source guard', () => {
  ...
  it('keeps Kepala Sub Bagian Umum dashboard folder-first and action-list based', () => {
    ...
    expect(source).toContain('Arsip Aktif')
    expect(source).toContain('Arsip Inaktif')
    expect(source).toContain('Usul Musnah')
    ...
    expect(source).toContain("query: { status_berkas: 'CLOSED', status_arsip: 'INAKTIF' }")
