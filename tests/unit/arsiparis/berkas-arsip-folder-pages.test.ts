...
describe('folder-first berkas archive page formatting', () => {
  ...
  it('resolves lifecycle buttons only for valid CLOSED folder statuses', () => {
    ...
    expect(resolveBerkasLifecycleAction('CLOSED', 'USUL_MUSNAH')?.confirmation)
      .toContain('metadata tetap tersimpan')
  ...
  it('keeps the active folder page constrained to open and active sections', () => {
    ...
    expect(detailSource).toContain('Pencarian lokal dokumen')
    expect(detailSource).toContain('filterBerkasDetailItems(items, searchQuery)')
    ...
    expect(inactiveSource).toContain('useNavigate')
    expect(inactiveSource).toContain("await navigate({ to: '/arsiparis/usul-musnah' })")
    ...
    expect(inactiveSource).toContain('Filter lokal berdasarkan Jenis Pembayaran, Nomor SPM, tanggal tutup, dan jumlah dokumen.')
    expect(inactiveSource).toContain('filterBerkasFolders(folders, searchQuery)')
    expect(inactiveSource).toContain('Export CSV')
    ...
    expect(inactiveExportSource).not.toContain('berkas_id')
    ...
    expect(proposedSource).toContain("from '#/components/ui/dialog'")
    expect(proposedSource).toContain('<Dialog')
    expect(proposedSource).toContain('<DialogContent')
    expect(proposedSource).toContain('<DialogTitle>Musnahkan Data</DialogTitle>')
    expect(proposedSource).toContain('openDestructionDialog')
    expect(proposedSource).toContain('closeDestructionDialog')
    ...
    expect(proposedSource).toContain('File fisik terkait berkas akan dihapus.')
    expect(proposedSource).toContain('Preview dan download file akan tetap diblokir.')
    expect(proposedSource).toContain('Metadata berkas dan dokumen tetap tersimpan.')
    expect(proposedSource).toContain('Aksi ini tidak mudah dibalik.')
    ...
    expect(proposedSource).toContain('Filter lokal berdasarkan Jenis Pembayaran, Nomor SPM, tanggal tutup, dan jumlah dokumen.')
    expect(proposedSource).toContain('filterBerkasFolders(folders, searchQuery)')
    expect(proposedSource).toContain('Export CSV')
    ...
    expect(proposedExportSource).not.toContain('berkas_id')
