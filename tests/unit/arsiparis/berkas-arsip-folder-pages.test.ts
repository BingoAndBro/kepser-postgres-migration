...
describe('folder-first berkas archive page formatting', () => {
  ...
  it.skip('keeps the active folder page constrained to open and active sections', () => {
    ...
    expect(detailSource).toContain("import { useAppToast } from '#/components/ui/AppToast'")
    expect(detailSource).toContain('const { showToast } = useAppToast()')
    expect(detailSource).toContain('showToast({')
    expect(detailSource).toContain('Berkas berhasil ditutup dan menjadi Arsip Aktif.')
    expect(detailSource).toContain('Harap lengkapi semua data yang diperlukan.')
