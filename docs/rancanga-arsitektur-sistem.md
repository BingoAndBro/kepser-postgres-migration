...
| **Validasi Skema** | "Zod, di setiap batas input berpayload JSON" | **Diperbaiki 25 Sept 2026.** Judul kotak dikembalikan ke "Validasi Skema" (bukan "Validasi masukan") agar konsisten dengan istilah yang dipakai di panah 5-6 dan prinsip KNF-07 ("validasi skema di setiap boundary") — "skema" lebih presisi karena Zod memang teknik validasi terhadap struktur data yang dideklarasikan, bukan sekadar pengecekan ad-hoc. Dua endpoint yang sebelumnya memvalidasi manual (`ketua-tim/index.ts` POST, `ketua-tim/kegiatan/$kegiatanId.ts` PATCH) kini pakai skema Zod baru di `src/lib/schemas/ketua-tim.ts`. Endpoint lain yang tampak "tanpa Zod" saat audit awal ternyata: (a) GET tanpa body — tidak relevan divalidasi skema, (b) upload berkas via `FormData` (`users/me.ts`, `manual-arsip/.../attachments.ts`) — Zod bukan alat yang tepat untuk data biner, divalidasi lewat pemeriksaan tipe/ukuran berkas, atau (c) sudah pakai Zod di dalam fungsi `lib/` yang dipanggilnya (`manual-arsip/$id.ts` → `updateManualArsipRecord` → `updateManualArsipSchema.safeParse`). Kesimpulan: 100% endpoint berpayload JSON terstruktur tervalidasi Zod |
...
| 3 | Rute API/server function → Peramban | **"aset aplikasi (JS/CSS) dan respons JSON"** | Hitam | Satu arah, siku | **Diperbaiki:** label sebelumnya "berkas aplikasi" diganti "aset aplikasi" (istilah standar web engineering untuk *static assets*) karena bertabrakan makna dengan "berkas" yang di seluruh dokumen ini dipakai untuk arti dokumen arsip pengguna (`berkas_arsip`). Ini arah **balik** (response) — sengaja dibuat sebagai panah **terpisah** dari panah nomor 2, bukan panah dua-arah pada garis yang sama, karena isinya beda: permintaan berisi JSON+cookie, sedangkan balasannya bisa berupa aset statis (bundel JS/CSS) atau data JSON. Ini juga alasan kenapa titik keluarnya dari kotak "Rute API" (b2), bukan dari kotak "Pemeriksaan permintaan" (b1) — sesuai kode, respons dirakit setelah melewati rute, bukan langsung dari lapisan pemeriksaan |
...
3. **Label panjang pada panah 2 dan 3** ("permintaan HTTP (JSON) + cookie sesi, via LAN" dan "aset aplikasi (JS/CSS) dan respons JSON") — ini teks terpanjang di seluruh gambar, cek apakah melebar sampai menabrak kotak Pengguna di atasnya.
...
- Panah 3 (respons balik) sebaiknya menyertakan bahwa aset statis JS/CSS dilayani Nitro sebagai static file server, bukan oleh kotak "Rute API" — pertimbangkan memisah label atau titik keluar panahnya.
...
6. Validasi Zod awalnya tidak konsisten (2 endpoint validasi manual: `ketua-tim/index.ts`, `ketua-tim/kegiatan/$kegiatanId.ts`) — **sudah diperbaiki** dengan skema baru `src/lib/schemas/ketua-tim.ts`. Setelah perbaikan, klaim "Zod di setiap batas input JSON" sekarang akurat 100% (endpoint GET dan upload berkas dikecualikan karena memang bukan kandidat Zod).
7. Label panah 3 "berkas aplikasi (JS/CSS)" diganti "aset aplikasi (JS/CSS)" — kata "berkas" di seluruh dokumen ini sudah dipakai untuk arti dokumen arsip pengguna (`berkas_arsip`), jadi memakainya lagi untuk bundel JS/CSS menimbulkan tabrakan makna dalam satu gambar yang sama.
