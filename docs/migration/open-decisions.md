...
- Phase 11H.2 P1 security gate decision framework, 11H.2a CSRF/origin selection, 11H.2b login rate-limit selection, 11H.2c cleanup hardening selection, and 11H.2d raw file-access hardening selection.
  Date: 2026-05-21.
  Rationale: `docs/migration/phase-11h-p1-security-gate-decision.md` records the decision framework for CSRF/origin strategy, login rate-limit/brute-force foundation, destructive admin cleanup hardening, and raw logical-path file-access hardening. The human selected incremental P1 implementation and chose CSRF/origin protection, login rate-limit/brute-force protection, destructive admin cleanup hardening, and raw logical-path file-access hardening. Phase 11H.2a is implemented pending human retest: unsafe API methods now use centralized Origin/Referer same-origin protection. Phase 11H.2b is implemented pending human retest: login now uses an in-memory local-process limiter with 5 failed attempts per normalized identifier/IP key in 10 minutes and a 15-minute cooldown. Phase 11H.2c is implemented pending human retest: GET cleanup is dry-run/report-only, and destructive cleanup requires POST body `dry_run=false` plus `confirm=true`. Phase 11H.2d is implemented pending human retest: raw logical-path preview/download and `/api/files/access` revalidate current document/archive references and block `DIMUSNAHKAN` at token-use time. This does not accept bounded risk, downgrade P1 items, approve 11H.3, or make a final readiness decision.
- Phase 11H.3 final release handoff classification.
  Date: 2026-05-22.
  Rationale: `docs/migration/phase-11h-final-release-classification.md` classifies the migration as `Partial / bounded release handoff: local/LAN target is ready for human-controlled internal handoff, subject to deployment posture decisions and accepted limitations.` The P1 follow-ups from 11H.2a/11H.2b/11H.2c/11H.2d are implemented and human-smoked. Active runtime/package Supabase dependency is retired, while historical docs/tests/comments/env-example references and the retained `supabase/` folder remain traceability or cleanup backlog. This classification does not approve public production readiness, broad LAN readiness, operational certification, compliance validation, full Supabase repository removal, or go-live.
...
- Exact CSRF/origin enforcement design for cookie-authenticated state-changing routes before 11H/wider rollout.
  ...
  Phase 11H.2a status: implementation selected and implemented pending human retest. Unsafe API methods now require same-origin `Origin` or same-origin `Referer` fallback; missing both is rejected by default. Final classification still requires human retest review.
- Exact app-layer rate-limit design before 11H/wider rollout.
  ...
  Phase 11H.2b status: implementation selected and implemented pending human retest. Login now has an in-memory local-process limiter keyed by normalized identifier plus IP when available, with 5 failed attempts per 10 minutes and a 15-minute cooldown. Limiter state resets on process restart and is not distributed; stronger reverse-proxy and/or persistent limits remain future hardening depending on final topology. Upload, file-token issuance, workflow, archive, admin, and other sensitive-route throttling remain P2/P1 follow-up decisions by exposure.
- Destructive admin storage cleanup POST-only hardening.
  Phase 11G.5 note: cleanup defaulted to dry-run and required ADMIN, but destructive mode was GET-triggered when query flags disabled dry-run.
  Phase 11H.2c status: implementation selected and implemented pending human retest. GET cleanup is now dry-run/report-only, including with destructive query flags. Destructive cleanup requires POST body `dry_run=false` and `confirm=true`; pending cleanup additionally requires `include_pending=true` and age eligibility. This does not resolve the separate app-wide CSRF/origin P1.
- Whether raw logical-path preview/download compatibility routes should become owner-only, document-token-only, or status-aware before final rollout.
  ...
  Phase 11H.2d status: implementation selected and implemented pending human retest. Raw logical-path preview/download and `/api/files/access` now revalidate current document/archive references, block `DIMUSNAHKAN` at token-use time, keep pending paths owner-scoped, and keep ungoverned paths owner-scoped. Final classification still requires human retest review.
- Final 11H P1 security decision.
  Phase 11H.3 note: decision framework is recorded and the four P1 follow-ups are implemented and human-smoked. The remaining open part is not the P1 implementation status; it is future hardening depth if deployment expands, including persistent/distributed rate limiting, reverse-proxy throttling, full CSRF token framework, and broader mutation throttling.
