...
Phase 11H.3 status update: `docs/migration/phase-11h-final-release-classification.md` records the final classification as partial / bounded release handoff for human-controlled internal/local/LAN use. The P1 follow-ups in 11H.2a CSRF/Origin Protection, 11H.2b Login Rate-Limit/Brute-Force, 11H.2c Destructive Admin Cleanup Hardening, and 11H.2d Raw Logical-Path File Access Hardening are implemented and human-smoked. Unsafe API methods use centralized Origin/Referer same-origin protection, login has an in-memory local-process limiter with 5 failed attempts per normalized identifier/IP key in 10 minutes and a 15-minute cooldown, GET cleanup is dry-run/report-only and destructive cleanup requires POST body `dry_run=false` plus `confirm=true`, and raw logical-path preview/download plus `/api/files/access` revalidate current document/archive references and block `DIMUSNAHKAN` at token-use time. The next recommended state is human-controlled maintenance/backlog governance, not public production or go-live approval.
...
Already-local write surfaces are not Phase 8 runtime targets: `POST /api/dokumen/submit` is local-backed after Phase 6G.6, `POST /api/upload` is local upload-backed, and `POST /api/dokumen/rename-pending` now uses local pending-to-formal movement plus a local Drizzle document ownership lookup after 11D.2c. They remain relevant to storage validation but should not be reworked in Phase 8A/8B.
...
|---|---|---|---|---|---|---|
| `POST /api/dokumen/` draft-create branch | Historical 11A/11D.1 match; no longer imports `createServerSupabaseClient`, `getServerSession as getSession`, or `createDokumen` after 11D.2a. | Migrated local-backed route branch. | `GET /api/dokumen/` and `POST /api/dokumen/` now use local `dms_session` and local Drizzle within the mixed route file. | Do not delete helpers solely from this row; use current source grep and 11D.3 export-level audit. | 11D.2a complete; defer helper cleanup to 11D.3/11E. | 11D. |
| `PATCH /api/dokumen/$id/nominal` | Historical 11A/11D.1 match; no longer imports `createServerSupabaseClient`, `getServerSession`, or Supabase-backed `insertLog` after 11D.2b. | Migrated local-backed route. | The route now uses local `dms_session`, local Drizzle document/archive lookups, local nominal update, and local append-only audit insert. | Do not delete helpers solely from this row; use current source grep and 11D.3 export-level audit. | 11D.2b complete; defer helper cleanup to 11D.3/11E. | 11D. |
| `POST /api/dokumen/rename-pending` document lookup | Historical 11A/11D.1 match; no longer imports `createAdminClient` or Supabase-backed `getDokumenById` after 11D.2c. | Migrated local-backed lookup. | Auth, ownership lookup, and storage movement are local-backed through `getLocalServerSession`, narrow Drizzle `dokumen_transaksi` lookup, and local pending movement helpers. | Do not delete helpers solely from this row; use current source grep and 11D.3 export-level audit. | 11D.2c complete; defer helper cleanup to 11D.3/11E. | 11D. |
...
| Add custom user docs | Historical 11C behavior created direct `user-custom-{crypto.randomUUID()}` entries in `userDocs`; 11G.4b supersedes this with `createClientId('user-custom')` for HTTP LAN browser compatibility. Upload for the custom doc uses the same replacement flow and lampiran metadata. |
...
| `POST /api/dokumen/rename-pending` | Local movement already implemented; document lookup is local Drizzle-backed after 11D.2c | Not needed for `AttachmentEditor` reset/cancel cleanup. Do not use as browser cleanup route. |
...
- Custom user docs still use `user-custom-*` temporary client ids. 11G.4b now generates them through `createClientId('user-custom')` instead of direct browser `crypto.randomUUID()` for HTTP LAN compatibility. Removing a custom user doc remains blocked while it has an uploaded lampiran or pending replacement.
...
- User-created optional document behavior remains unchanged, including `user-custom-*` temporary client ids, add/remove title behavior, and removal from `lampiranUrls`. 11G.4b now generates those ids through `createClientId('user-custom')` instead of direct browser `crypto.randomUUID()` for HTTP LAN compatibility.
...
| `src/lib/supabase-server.ts` | No longer imported by `src/routes/api/dokumen/index.ts` after 11D.2a or `src/routes/api/dokumen/$id.nominal.ts` after 11D.2b. | Helper/dead-code cleanup candidate, subject to full caller re-audit. | No. | Defer deletion to 11D.3/11E after final source/test/doc/package audit. |
| `src/lib/supabase-admin.ts` | No longer imported by `src/routes/api/dokumen/rename-pending.ts` after 11D.2c; older tests/docs may still reference Supabase helper names. | Helper/dead-code cleanup candidate, subject to full caller re-audit. | No. | Defer deletion to 11D.3/11E after final source/test/doc/package audit. |
| `src/lib/auth.ts` | No longer imported by the scoped 11D.2 dokumen routes after 11D.2a/11D.2b; internally wraps `supabase.auth.getSession()`, `supabase.auth.getUser()`, role reads, and active-role cookie helpers. | Helper/dead-code or split candidate, subject to full caller re-audit because cookie/helper exports may still be referenced elsewhere. | No. | Later split/retire Supabase session helpers only after checking all imports and cookie-helper needs. |
...
| `src/lib/dokumen/mutations.ts` | Supabase-backed `createDokumen`, `updateDokumen`, `updateDokumenStatus`; `createDokumen` is no longer active through `POST /api/dokumen` after 11D.2a. | Legacy helper/dead-code candidate, not an active `POST /api/dokumen` dependency after 11D.2a. | No. | Defer export-level caller validation and cleanup to 11D.3. |
| `src/lib/dokumen/queries.ts` | Supabase-backed reads; `getDokumenById` is no longer imported by `POST /api/dokumen/rename-pending` after 11D.2c. Other query functions have no active runtime callers in current grep and must be rechecked before deletion. | Legacy helper/dead-code candidate, not an active `rename-pending` dependency after 11D.2c. | No. | Defer export-level caller validation and cleanup to 11D.3. |
| `src/lib/dokumen/logs.ts` | Supabase-backed `insertLog` and `getLogsByDokumen`; `insertLog` is no longer active through `PATCH /api/dokumen/$id/nominal` after 11D.2b. | Legacy helper/dead-code candidate, not an active nominal-route dependency after 11D.2b. | No. | Defer export-level caller validation and cleanup to 11D.3. |
...
- `src/lib/dokumen/mutations.ts` is no longer active through legacy `createDokumen` on `POST /api/dokumen` after 11D.2a. Its Supabase mutation exports are candidate unused legacy helpers by current caller grep, but must be rechecked in 11D.3 before deletion.
- `src/lib/dokumen/queries.ts` is no longer active through `POST /api/dokumen/rename-pending` after 11D.2c. Its Supabase query exports are candidate unused legacy helpers by current caller grep, but must be rechecked in 11D.3 before deletion.
- `src/lib/dokumen/logs.ts` is no longer active through `PATCH /api/dokumen/$id/nominal` after 11D.2b, but remains a deferred helper/dead-code cleanup candidate that must be re-audited before deletion.
...
- `tests/unit/dokumen/submit-route-parity.test.ts`, `tests/unit/storage/raw-preview-internal-url-runtime.test.ts`, and `tests/unit/storage/raw-preview-internal-url-wiring.test.ts` contain mocks or assertions for Supabase helper names. `tests/unit/storage/rename-pending-local-route.test.ts` now mocks the local Drizzle lookup used by `rename-pending`. Classification: tests/mock reference, not live runtime dependency.
...
- `src/lib/supabase-server.ts`: no active scoped 11D.2 dokumen callers after 11D.2a/11D.2b, but do not delete until the full 11D.3/11E source/test/doc/package audit confirms no remaining active or reference requirements.
- `src/lib/supabase-admin.ts`: no longer called by `POST /api/dokumen/rename-pending` after 11D.2c, but do not delete until the full 11D.3/11E source/test/doc/package audit confirms no remaining active or reference requirements.
- `src/lib/auth.ts`: no active scoped 11D.2 dokumen callers after 11D.2a/11D.2b, but do not delete until import-level audit confirms whether non-Supabase cookie/helper exports are still needed.
- `src/lib/dokumen/mutations.ts`: no active `POST /api/dokumen` caller after 11D.2a, but remains a deferred helper/dead-code cleanup candidate that must be re-audited before deletion.
- `src/lib/dokumen/queries.ts`: no active `rename-pending` caller after 11D.2c, but remains a deferred helper/dead-code cleanup candidate that must be re-audited before deletion.
- `src/lib/dokumen/logs.ts`: no active nominal-route caller after 11D.2b, but remains a deferred helper/dead-code cleanup candidate that must be re-audited before deletion.
...
- 11D.2c is now complete: `POST /api/dokumen/rename-pending` uses a local Drizzle document lookup while preserving local auth and local pending movement behavior.
...
- Expected broader audit after this phase and subsequent 11D.2b/11D.2c work: `src/routes/api/dokumen/$id.nominal.ts` and `src/routes/api/dokumen/rename-pending.ts` should no longer have active route-level Supabase dependencies.
...
- 11D.2c is now complete: `POST /api/dokumen/rename-pending` no longer has a Supabase admin/helper-backed document lookup, while its auth and local pending movement behavior remain local-backed.
...
- Expected broader audit after this phase and subsequent 11D.2c work: `src/routes/api/dokumen/rename-pending.ts` should no longer have active route-level Supabase dependencies.

#### Phase 11D.2c: Resolve POST /api/dokumen/rename-pending Supabase Runtime Dependency

Date: 2026-05-19.

Status: scoped runtime/docs migration complete for the document lookup inside `POST /api/dokumen/rename-pending` in `src/routes/api/dokumen/rename-pending.ts`. No commit was made.

Runtime decision:

- Chosen path: narrow local migration of the document ownership lookup only.
- The route path and method remain `POST /api/dokumen/rename-pending`.
- The request body remains the compatibility body `{ dokId, lampiranUrls, userId? }` with passthrough parsing.
- The response/error shapes and status categories are preserved as closely as possible: invalid JSON, missing fields, unauthenticated, unauthorized, missing document, invalid path, missing source, target conflict, controlled move failure, and success still use the existing JSON wrappers.
- No route tree, UI caller, submit route, update route, resubmit route, storage helper, package, env, migration, seed, or DB schema behavior changed.

Local document lookup behavior:

- Removed `createAdminClient` and Supabase-backed `getDokumenById(...)` usage from `src/routes/api/dokumen/rename-pending.ts`.
- Added a route-local Drizzle lookup against `dokumen.dokumen_transaksi` that selects only `id` and `created_by`.
- The lookup returns `null` for no row and for lookup errors, preserving the closest legacy helper behavior where `getDokumenById(...)` returned `null` on Supabase errors or missing data.
- Missing document behavior remains `404 { error: 'Dokumen tidak ditemukan' }`.

Auth/RBAC/ownership behavior:

- The route still uses `getLocalServerSession(request)` and does not trust `dms_active_role` alone.
- Unauthenticated remains `401 { error: 'Unauthorized' }`.
- Body `userId` remains compatibility input and must match the local session user id; mismatch remains `403 { error: 'Anda tidak memiliki akses' }`.
- Document ownership remains owner-only by checking `dokumen_transaksi.created_by === session.userId`.
- No new role grant was added for `ADMIN`, `ARSIPARIS`, `PPK`, or `BENDAHARA`.

Local pending movement/path safety preservation:

- Local pending movement still uses the existing `classifyLocalPendingMovePath(...)`, `generateLocalFormalTargetLogicalPath(...)`, and `moveLocalPendingFileToFormal(...)` orchestration.
- Pending underscore and dash path compatibility is unchanged.
- Already formal and safe unsupported paths are still skipped.
- Source owner/path checks, target path generation, target conflict behavior, missing source behavior, no-overwrite semantics, physical root containment, and logical-only returned paths remain owned by the existing local storage helpers.
- No Supabase Storage fallback was added.

Testing/docs status:

- `tests/unit/storage/rename-pending-local-route.test.ts` was updated only to mock the local Drizzle lookup chain instead of `createAdminClient`/`getDokumenById`.
- No broad tests, build, full typecheck, dev server, DB scripts, migrations, seeds, route generation, package commands, Playwright/E2E, or commit were run for this phase.
- Phase 11D.2a remains complete for `POST /api/dokumen`.
- Phase 11D.2b remains complete for `PATCH /api/dokumen/$id/nominal`.
- After 11D.2c, no active route-level Supabase dependency should remain in `src/routes/api/dokumen/rename-pending.ts`. Remaining Supabase helper references are helper/dead-code cleanup debt for 11D.3/11E unless a later audit identifies an active runtime caller.
...
#### Phase 11D.4: Post-Retirement Audit And Phase 11E Handoff

Date: 2026-05-19.

Status: audit/docs handoff complete. No commit was made.

Scope:

- Audit-only verification after Phase 11D.3 helper/source cleanup.
- No runtime source, test, package, env, route tree, DB, drizzle, migration, seed, script, or supabase-folder changes.
- No package/env cleanup, dependency removal, route generation, heavy validation, old Supabase data/file migration, copy, download, backfill, sync, or recovery.
...
- `git diff --check`
- `git diff --name-only`
- Targeted `git grep` over `src`, `tests`, `docs/migration`, package files, lockfile, and env files for Supabase factories, direct Supabase auth/database/storage calls, Supabase package imports, env names, legacy helper imports, local auth usage, route paths, and preserved pure/type helper imports.
- Protected diff checks for `src`, `tests`, `src/routeTree.gen.ts`, `package.json`, `pnpm-lock.yaml`, `.env`, `.env.migration`, `db`, `drizzle`, and `supabase`.

Final audit result:

- No active source/runtime match was found for `createServerSupabaseClient`, `createAdminClient`, `getBrowserClient`, `createBrowserClient`, `supabase.auth`, `auth.admin`, `supabase.from`, `supabase.storage`, `storage.from`, or `SupabaseClient` in the audited `src`/`tests` runtime scope.
- No active source/test import from `@supabase/ssr` or `@supabase/supabase-js` was found. Remaining package names are in `package.json` and `pnpm-lock.yaml` only.
- No active source/test/docs import from the deleted legacy helper modules was found for `#/lib/supabase*`, `@/lib/supabase*`, root `#/lib/auth`, root `@/lib/auth`, `#/lib/user-helpers`, `@/lib/user-helpers`, `#/lib/dokumen/{mutations,queries,logs}`, `@/lib/dokumen/{mutations,queries,logs}`, or deleted `#/lib/master-data/{fungsi,kegiatan,jenis,kategori,detail,kelengkapan,jenis-dokumen}` modules. Remaining matches are docs/history command examples only.
- `src/lib/constants/env.ts` still contains `SUPABASE_URL`, `SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, `VITE_SUPABASE_URL`, and `VITE_SUPABASE_ANON_KEY` constants. A targeted source check found these Supabase env names only in that constants file and no `ENV_KEYS.SUPABASE*` use elsewhere in `src`; classify this as env cleanup deferred to 11E, not an active Supabase runtime dependency.
- Absence of grep/runtime matches is strong evidence for active source/runtime helper retirement, not an absolute proof against future config-driven or indirect loading patterns. No such indirect active pattern was found in this audit.

Remaining Supabase references by category:

| Category | Evidence | Classification | 11E handling |
|---|---|---|---|
| No source/runtime match | Runtime-source grep over factories, direct auth/database/storage calls, and `SupabaseClient` returned no `src`/`tests` matches. | Phase 11D source/helper retirement complete for verified active runtime dependencies. | Keep as 11E precondition; rerun before package/env cleanup. |
| Package dependency deferred to 11E | `package.json` still lists `@supabase/ssr` and `@supabase/supabase-js`. | Package dependency artifact only. | Remove only in 11E under human-approved package cleanup. |
| Lockfile dependency deferred to 11E | `pnpm-lock.yaml` still contains root dependency entries and transitive Supabase package resolution blocks. | Lockfile artifact only. | Update only through approved package install/update/remove workflow in 11E. |
| Env reference deferred to 11E | `.env` contains `VITE_SUPABASE_URL`, `SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`, `SUPABASE_ANON_KEY`, and `SUPABASE_SERVICE_ROLE_KEY`; `src/lib/constants/env.ts` contains matching constant names; docs mention historical/env examples. | Env/config/documentation artifact. `.env` is local/sensitive and must not be edited automatically. | Clean env examples/docs/constants only if 11E confirms no runtime requires them; do not edit `.env` or `.env.migration` without explicit human approval. |
| Docs/history reference | `docs/`, especially migration docs and old specs/best-practice notes, retain Supabase terms, commands, historical phase notes, and examples. | Useful migration history or legacy reference. | Audit for misleading current-runtime wording; preserve useful history unless a docs hygiene task explicitly scopes deletion. |
| Test/mock reference | Required greps found no active Supabase package/helper runtime import in `tests`. Historical docs mention old mocked Supabase route-test plans. | No current test blocker found by this audit. | Recheck in 11E before dependency removal. |
| Blocker/unclear | None found by the required grep/protected-diff audit. | No narrow 11D.x blocker is required before 11E. | If manual review finds runtime indirection not covered by grep, split a narrow 11D.x follow-up before package/env cleanup. |

Pure/type helper preservation audit:

- `src/lib/dokumen/index.ts` still exports parse helpers and pure filename/path helpers only.
- `src/lib/dokumen/storage.ts` still provides `buildStorageFilename`, `buildDokumenFilename`, `storagePathBelongsToUser`, and `isStoragePathPending` without Supabase client imports or storage/database calls.
- `src/lib/master-data/index.ts` re-exports only `shared.ts`.
- `src/lib/master-data/shared.ts` preserves row types such as `FungsiRow`, `KegiatanRow`, `KelengkapanRow`, and `JenisDokumenRow`.
- `src/lib/user-response.ts` validates role arrays through `roleArraySchema` directly and no longer imports the deleted root auth helper.

Route/runtime grep audit:

- `/api/dokumen/submit`, `/api/dokumen/rename-pending`, `/api/dokumen/$id/nominal`, `/api/files/access`, and `/api/upload` remain registered/local runtime surfaces in `src/routes` and caller references.
- The audited API routes use `getLocalServerSession(...)` and `hasLocalRole(...)` patterns under the local auth boundary where authorization is required.
- `dms_session` and `dms_active_role` references are concentrated in local auth/session docs and source; `dms_active_role` remains UX state only and server checks must continue to validate assigned roles.

Phase 11D completion verdict:

- Phase 11D source/helper retirement is complete for verified active source/runtime Supabase helper dependencies.
- This verdict does not claim package cleanup, lockfile cleanup, env cleanup, historical docs cleanup, full regression, release hardening, backup/restore, LAN readiness, or old Supabase data/file migration.
- Old Supabase production/current data and old Supabase Storage files remain intentionally not migrated, copied, downloaded, backfilled, synced, or recovered.

Phase 11E handoff scope:

- Remove `@supabase/ssr` and `@supabase/supabase-js` from `package.json` and `pnpm-lock.yaml` only if a fresh 11E audit again confirms no source/test package dependency remains.
- Clean `SUPABASE_*` and `VITE_SUPABASE_*` env documentation/references only if no runtime requires them. Treat `.env` and `.env.migration` as local/sensitive and do not edit them automatically.
- Audit `src/lib/constants/env.ts` and any env-reference docs/examples as part of 11E. Remove or narrow Supabase constants only if no active code uses them.
- Preserve useful historical migration references in docs/specs; delete or rewrite only misleading current-runtime instructions when explicitly scoped.
- Run package install/update/remove only under a human-approved 11E prompt. If package/env/docs cleanup risks diverge materially, split 11E into narrower cleanup slices instead of broad cleanup execution.
- Do not add Supabase fallback, restore old Supabase helpers, migrate/copy/backfill/recover old Supabase data/files, or claim Phase 11E completion from this 11D.4 audit.

Validation notes:

- Lightweight validation only was used for this phase.
- No `pnpm build`, broad `pnpm test`, full typecheck, dev server, DB scripts, migrations, seeds, route generation, package install/remove/update, dependency cleanup, Playwright/E2E, or commit was run.
- Manual validation after review may run `pnpm test`, and `pnpm build` only if the human chooses.
...
- Phase 11F.3 found a production preview runtime blocker. Phase 11F.4a resolved the `pg-native` optional dependency crash and Phase 11F.4b resolved the separate preview UI JSX runtime blocker, but Phase 11G remains deferred until follow-up production-like smoke and long-session performance validation is recorded.
...
Status: targeted runtime fix complete for the original `pg-native` blocker. Phase 11F remains open because preview UI rendering now fails on a separate JSX dev-runtime issue.

Fix summary:

- Externalized only `pg-native` in the Nitro server Rollup config.
- Preserved local PostgreSQL/Drizzle runtime through `pg`'s normal JavaScript client path.
- Did not install `pg-native`, change packages, change env files, change DB schema/migrations/seeds/scripts, reintroduce Supabase, regenerate routes, or change DB query semantics.

Validation evidence:

- `pnpm build` passed.
- Built server output no longer contains Vite's generated `Could not resolve "pg-native"` throw.
- `pnpm preview` started without the `pg-native` crash. Port `3000` was occupied, so preview selected `http://localhost:3001/`.
- `GET /api/master-fungsi` returned `200` in preview and exercised the local PostgreSQL/Drizzle path.
- `GET /` returned `500` with `jsxDevRuntimeExports.jsxDEV is not a function` from built `RootDocument`; this is tracked as the next production-preview blocker.
...
Status: targeted runtime fix complete for the `jsxDEV` RootDocument blocker. Phase 11F remains open for follow-up production-like smoke and long-session performance validation.

Fix summary:

- Forced the Vite esbuild JSX transform to emit production JSX runtime calls with `esbuild.jsxDev: false`.
- Kept the Phase 11F.4a `pg-native` Nitro externalization unchanged.
- Did not install packages, change DB connection semantics, change route registration, modify `src/routeTree.gen.ts`, edit env files, or reintroduce Supabase runtime behavior.

Validation evidence:

- `pnpm build` passed.
- Built SSR chunks no longer contain `jsxDEV`, `jsxDevRuntimeExports`, or `react/jsx-dev-runtime` references.
- `pnpm preview` started on fixed validation port `http://localhost:3017/`.
- `GET /` returned `200`.
- `GET /api/master-fungsi` returned `200`, confirming local PostgreSQL/Drizzle still works in preview.
- The prior `pg-native` crash did not return.
...
Status: bounded audit complete. No runtime source fix was made. This is not final release readiness or final performance certification because authenticated browser DevTools long-session checks were not run.

Validation evidence:

- `pnpm build` passed and again reported TanStack devtools code removal from `src/routes/__root.tsx`.
- `pnpm preview` was run through a temporary PowerShell job on fixed port `http://127.0.0.1:3018/`.
- Preview HTTP probes returned `200` for `/`, `/api/auth/session`, and `/api/master-fungsi`.
- A direct preview probe to `/__tsd/console-pipe` returned `404`.
- Built output grep found no `__tsd`, `console-pipe`, TanStack devtools runtime names, `react/jsx-dev-runtime`, `jsxDEV`, or `jsxDevRuntimeExports`.
- A 60-second idle wait did not expose a preview crash; browser-initiated idle requests were not observable in this HTTP-only probe.

Audit classification:

- `/__tsd/console-pipe*` is classified as dev-only tooling overhead based on preview absence and build stripping.
- Chrome Network growth during a long dev session remains likely expected accumulation unless requests continue while idle.
- Duplicate page fetches are classified as non-blocking P1 optimization candidates. Ajukan Dokumen has one `/api/master-fungsi` mount effect and no matching route loader, so a two-call dev observation is most likely dev remount/tooling behavior unless preview browser smoke reproduces it.
- No active `setInterval`, `EventSource`, or `WebSocket` usage was found; audited event listeners have cleanup; no source-level memory/listener leak was proven.
...
- Classification API database validation/write failures now return controlled JSON without changing endpoint paths, request payloads, or success shapes. Phase 12I.4 changes mutation authorization to Kepala Sub Bagian Umum only.
...
Goal: delete superseded local formal attachment files only after edit/revisi/save/resubmit persistence succeeds, while preserving old files on cancel, reset, validation failure, move failure, or DB failure.

Allowed scope:

- Attachment replacement lifecycle in `AttachmentEditor`, Pegawai edit/revisi, and PPK resubmit routes.
- Server-side post-persistence cleanup for old formal files no longer referenced by current persisted metadata or archive snapshots.
- Focused local storage helper tests and manual smoke checklist updates.

Non-goals:

- No broad storage redesign, global orphan cleanup redesign, archive lifecycle redesign, DB schema changes, route generation, package/env changes, Supabase fallback, or old Supabase file migration/copy/download/backfill/sync.

Validation gates:

- Existing formal file remains while editing and after cancel/reset before submit.
- Pending replacement is formalized through the existing local move path before DB metadata points to it.
- Old replaced formal file is deleted only after successful DB persistence and only when no current document/archive metadata reference protects it.
- Submit/update/resubmit failure leaves old formal files intact.
- Missing old local files and legacy/unsafe paths are skipped cleanly without physical path leakage.

Manual smoke checklist:

- Replace one attachment and cancel/reset before submit; confirm the old file remains previewable.
- Replace one attachment and submit successfully; confirm metadata points to the new file and the old file is gone.
- Trigger a validation/submit failure after selecting a replacement; confirm the old file remains.
- Repeat through PPK resubmit if available.
- Confirm archive/DIMUSNAHKAN preview/download behavior is unchanged.

Implementation update on 2026-05-20:

- Targeted fix implemented; pending human browser/storage retest.
- `AttachmentEditor` continues to clean up only pending replacement uploads on reset/cancel and does not delete old formal files during selection.
- Pegawai edit/revisi and PPK resubmit now run old-file cleanup only after successful DB update/transaction.
- Cleanup compares old persisted paths with the newly persisted paths and a DB-loaded reference guard over current document attachment metadata plus retained archive snapshots.
- Cleanup deletes only safe local formal paths, treats missing files as a no-op, skips pending/unsafe/legacy paths, and logs only safe counts/codes on cleanup failure.

#### Phase 11F.5d.1: Superseded Pending Replacement Cleanup

Goal: clean pending replacement uploads that are superseded inside the same edit/revisi/resubmit session before submit/cancel, without deleting old formal files before successful persistence.

Allowed scope:

- Session-scoped pending upload tracking in `AttachmentEditor`.
- Existing `POST /api/upload?cleanup=pending` pending-only cleanup behavior.
- Focused helper tests and manual smoke checklist updates.

Non-goals:

- No broad upload-session manager, global orphan cleanup framework, DB schema change, route generation, package/env change, Supabase fallback, or archive lifecycle redesign.

Implementation update on 2026-05-20:

- Targeted fix implemented; pending human browser/storage retest.
- `AttachmentEditor` now tracks every pending upload URL created during the active editor session, not only the latest pending file per kelengkapan id.
- Replacing the same attachment again cleans the previous pending URL when it is no longer referenced by current form state.
- Reset and cancel cleanup now use the session-created pending URL set, so multiple replacement attempts in one edit session are cleaned without touching old formal files.
- Submit attempts clean definitely unreferenced superseded pending uploads before persistence so immediate post-save navigation cannot strand them; success clears session tracking, and failed submit preserves the currently selected pending replacement for retry.
- Cleanup remains routed through the existing pending-only API, which rejects formal, unsafe, owner-mismatched, and unsupported paths before filesystem mutation.

#### Phase 11F.5d.2: Admin Storage Orphan Cleanup Diagnostics Hardening

Goal: harden admin-only local storage diagnostics and cleanup so orphan analysis is accurate for current local filesystem storage and eligible orphan files can be cleaned without deleting referenced, active, recent-pending, archived, or unsafe paths.

Allowed scope:

- Existing admin storage diagnostic routes.
- Local storage diagnostics helper.
- Focused storage diagnostics tests.
- Regression smoke documentation.

Non-goals:

- No broad storage redesign, DB schema or migration change, archive lifecycle redesign, route generation, package/env cleanup, Supabase fallback, or old Supabase data/file migration/copy/download/backfill/sync.

Implementation update on 2026-05-20:

- Targeted fix implemented; pending human browser/storage retest.
- `GET /api/admin/analyze-storage` continues to return logical paths only and now includes pending path age details plus default pending cleanup eligibility counts.
- `GET /api/admin/cleanup-orphan-files` keeps `dry_run=true` as the default and still deletes non-pending formal orphan candidates only when `dry_run=false`.
- Pending cleanup is explicit only: pending deletion requires `dry_run=false`, `include_pending=true`, `confirm=true`, and the pending file must meet `min_age_minutes` threshold, defaulting to 1440 minutes.
- Recent pending files are reported/skipped by default so active edit/revisi/resubmit form sessions are not cleaned blindly.
- Cleanup candidates remain derived from DB-protected analysis over current document attachment metadata plus retained archive snapshots; unsafe, unsupported, legacy, and missing paths are skipped or no-op without physical path exposure.
...
- Targeted fix implemented; pending human browser retest.
- `/pegawai/dokumen` now has a parent assigned-PEGAWAI route guard and a blocking client role check before child document pages render.
- Admin and other non-PEGAWAI users should be redirected to `/forbidden` instead of seeing the document-list page's raw `/api/dokumen` 403 fetch error.
- `GET /api/dokumen` remains unchanged and still requires assigned `PEGAWAI` plus owner filtering.
- Guard dev logs now label `userRoles`, `activeUxRole`, and `requiredRouteRole` separately so required route roles are not presented as actual user roles.
- Phase 11F.5g.1 cleaned the Pegawai wrapper fallback after human retest found a React warning during the forbidden redirect. The wrapper no longer fetches auth or updates local state; it gates child rendering from client auth state and keeps redirect side effects inside `useEffect`.

#### Phase 11F.5h: Pegawai Canonical Route Alignment

Goal: make `/pegawai` the canonical Pegawai dashboard/default route while preserving `/` compatibility and role-based defaults.

Allowed scope:

- Existing root route compatibility handling.
- Existing `/pegawai` parent route and Pegawai dashboard component reuse.
- Default-route constants/navigation config.
- Login and role-switch default-route behavior where it already uses the shared mapping.
- Regression smoke documentation.

Non-goals:

- No `/pegawai/dokumen` rename.
- No route generation or `src/routeTree.gen.ts` edits.
- No API, DB, storage, package, env, auth-model, role-model, workflow, or FSM changes.
- No RBAC broadening and no implicit Admin access to Pegawai routes.

Implementation update on 2026-05-20:

- Targeted fix implemented; pending human route/browser retest.
- `ROLE_DEFAULT_ROUTE.PEGAWAI` now points to `/pegawai`.
- `/pegawai` reuses the existing Pegawai dashboard content and has an assigned-PEGAWAI guard; non-Pegawai users are redirected to `/forbidden` and missing sessions to `/login`.
- `/` now remains compatible as a default-route redirect that prefers a valid active UX role and otherwise falls back to the stable assigned-role order.
- Login success and role-switch defaults now route PEGAWAI to `/pegawai`; PPK, Bendahara, Arsiparis, and Admin defaults remain unchanged.
- `/pegawai/dokumen` remains a subroute, and server/API RBAC remains unchanged.
...
- Phase 11F.6 is complete as a docs/report recap only.
- 11F.5a through 11F.5h were implemented, human retested or accepted where reported, and committed before this recap.
- 11F.5d.2b pending-only dry-run response/message consistency is recorded as human-reported implemented/retested/committed as part of the admin storage cleanup hardening follow-up.
- Codex ran lightweight git/doc audits only during 11F.6 and did not run `pnpm test`, `pnpm build`, preview/dev server, E2E, DB scripts, route generation, package commands, or migrations/seeds.
- This status is not 11G evidence, production certification, LAN deployment completion, backup/restore completion, or final go-live readiness.
...
- Attachment replacement old-file cleanup.
- Kategori/Detail admin consistency.
- Accessibility pages touched by 11F.5f.
...
### Phase 11G: Operational Hardening Breakdown, Evidence, And Handoff

Goal: make the local/LAN deployment operationally safe.

Status:

- Phase 11G.0 completed the docs-only operational-hardening breakdown.
- Phase 11G.1 has added the docs-only backup/restore runbook at `docs/migration/phase-11g-backup-restore-runbook.md`.
...
|---|---|---|---|
| 11G.0 | Operational Hardening Breakdown Planning | Docs-only planning and audit. | 11G subphases are documented with guardrails and next phase selected. |
| 11G.1 | Operational Readiness Runbook And Backup/Restore Plan | Create runbook and evidence templates only. | Human has a safe backup/restore procedure to execute later. |
| 11G.2 | Preview Performance Baseline And Asset Hygiene Planning | Clean no-extension Lighthouse baseline and asset/performance classification procedure; implementation only if approved as a later tiny subphase. | Baseline procedure and evidence template are recorded. |
| 11G.2a | Human Clean Preview Performance Baseline Evidence | Human executes clean no-extension Lighthouse; docs record evidence only. | Baseline evidence, provenance, and next recommendation are recorded. |
| 11G.3 | Human-Run Backup/Restore Drill Evidence | Human executes backup/restore into a clean local target; docs record evidence. | DB/storage restore evidence and validation results are recorded. |
| 11G.4 | LAN Binding And Client Smoke Evidence | Human intentionally binds app to LAN and tests another LAN client. | Host/port/firewall/client smoke evidence is recorded. |
...
| 11G.4b | LAN HTTP Browser API Compatibility Fix | Narrow non-security client temporary ID fallback for additional kelengkapan. | Ajukan/Revisi additional kelengkapan can be added in HTTP LAN browser contexts without direct `crypto.randomUUID()` dependency. |
| 11G.5 | Cookie Auth, CSRF, Rate-Limit Security Review | Review cookie-auth security posture first. | State-changing route, CSRF, brute-force, and rate-limit posture are documented. |
| 11G.6 | Operations Rollback And Release Handoff | Consolidate rollback plan, operator checklist, known risks, and 11H handoff. | 11H has evidence, blockers, and accepted risks to review. |
| 11H | Final Release Readiness Gate And Supabase Retirement Decision | Human go/no-go decision. | Human records final decision; no automatic certification. |
...
- Clean preview performance baseline and asset hygiene classification.
- Cookie auth, CSRF, and rate-limit security review for state-changing routes.
- Rollback checklist and final 11H handoff.
...
- No backup/restore execution in 11G.0 or 11G.1.
- No LAN binding, firewall command, or security implementation in 11G.0 or 11G.1.
- No production, LAN, release, or go-live approval claim.

Candidate files to read/change:

- `docs/migration/phase-11g-operations-plan.md`
...
- Backup sets must include PostgreSQL dump, `storage/` files, timestamp/version metadata, commit hash, and storage root configuration reference without printing secrets or physical roots in public logs.
...
- Storage diagnostics must be run as dry-run/report-only after restore before any cleanup action.
- PostgreSQL port should not be exposed to the whole LAN without an explicit operational reason.
- LAN firewall planning should allowlist the app port only on trusted/private network.
- CSRF/rate-limit review must not rely on Supabase Auth protections.
- Preview performance baseline must exclude browser extensions and must not treat preview-server cache/compression findings as final deployment findings unless reproduced in the final serving setup.
- Large `/bps-logo.png` is a likely real asset hygiene item; Admin/role dashboard TBT remains P2 unless severe clean-preview lag is reproduced.
- Recent Lighthouse scores are environment-dependent evidence, not final deployment performance proof. A clean browser/no-extension baseline should be recorded before final 11G/11H decision, but it does not block 11G.1 docs/runbook work.

Validation gates:

- Human can create a backup.
- Human can restore into a clean local target.
...
- Unauthenticated POST, non-admin admin access, cookie flags, logout/password-change invalidation, and brute-force/rate-limit posture are reviewed.
...
- Exit when backup/restore, LAN, security review, performance baseline, rollback, and 11H handoff evidence are documented or blockers are explicitly listed.

#### Phase 11G.1: Operational Readiness Runbook And Backup/Restore Plan

Date: 2026-05-21.

Status: docs/runbook-only update. No backup, restore, LAN binding, firewall command, package command, DB command, route generation, test, build, preview, deployment, cleanup, source change, schema change, migration, seed, or commit is performed by this phase.

Output:

- `docs/migration/phase-11g-backup-restore-runbook.md` created as the operator-facing backup/restore runbook.
- `docs/migration/phase-11g-operations-plan.md` now links to the runbook and points the next recommended phase to 11G.2.

Runbook scope:

- purpose, scope, non-goals, and safety warnings;
- operator prerequisites;
- required backup artifacts;
- PostgreSQL custom-format and plain SQL backup templates;
- PostgreSQL custom-format and plain SQL restore templates;
- local storage archive/copy templates;
- backup manifest metadata template;
- restore-to-clean-target flow;
...
- 11G.3 evidence log template;
- known blockers and handoff to 11G.2/11G.3.

Important guardrails:

- Command examples are templates requiring human review before execution.
- Backup schedule and retention remain open; no mandatory rotation policy is invented here.
- Representative/sample document verification is acceptable unless a later phase explicitly requires exhaustive verification.
- File-count/hash manifests are optional operational enhancements, not 11G.1 blockers.
- Restore drills should use a clean target by default and should not overwrite the active runtime environment.
- LAN deployment remains informational only until 11G.4.
- A future successful backup/restore drill does not by itself approve the final 11H decision.
...
Rationale: the established 11G sequence records the clean no-extension performance baseline before the human-run backup/restore drill, so recent Lighthouse concerns are classified before restore evidence is treated as final operational input.

#### Phase 11G.2: Preview Performance Baseline And Asset Hygiene Planning

Date: 2026-05-21.

Status: docs/planning-only update. No Lighthouse, build, preview, test, package command, DB command, Docker command, firewall command, backup/restore command, route generation, deployment command, source change, runtime change, asset replacement, cache/compression change, or commit is performed by this phase.

Output:

- `docs/migration/phase-11g-performance-baseline-plan.md` created as the operator-facing clean preview Lighthouse baseline and asset hygiene plan.
- `docs/migration/phase-11g-operations-plan.md` now links to the baseline plan and points the next recommended phase to human-run 11G.2a evidence unless the human chooses 11G.3 first.

Baseline plan scope:

- purpose, scope, non-goals, and safety warnings;
- why prior Lighthouse scores fluctuated;
- clean browser/no-extension baseline environment;
- human-only command templates for `pnpm build` and `pnpm preview`;
- role/page matrix with a manageable minimum baseline;
- metrics to record;
- evidence table template;
- `/bps-logo.png` asset hygiene classification;
- `pnpm preview` cache/compression classification as deployment-server concerns unless reproduced in final serving;
- TBT/main-thread classification and escalation thresholds;
- internal decision thresholds for 11G/11H inputs;
- handoff to 11G.2a or 11G.3.

Important guardrails:

- Thresholds are internal operational guidance, not public certification or SLA.
- Avoid cherry-picking isolated unusually high scores; prefer median or consistent repeated measurements.
- Browser extensions, VPN/proxy tools, and technology-profiler extensions must be absent from official evidence.
- Thermal throttling, unstable machine conditions, and heavy background tasks must be recorded if present.
- Large `/bps-logo.png` is likely a real asset hygiene item, but 11G.2 does not replace or optimize assets.
- Cache/compression findings may depend on the final serving topology outside the app runtime.
- Dashboard TBT around 400-600 ms is P2 deferred optimization unless clean evidence repeatedly shows severe lag, Performance below 60, TBT above 1000 ms, request loops, or unbounded growth.
...
Rationale: clean preview Lighthouse evidence is now recorded and acceptable for internal operational input, so the next operational phase is backup/restore drill evidence.

#### Phase 11G.2a: Human Clean Preview Performance Baseline Evidence

Date: 2026-05-21.

Status: evidence recorded. Human-provided clean preview Lighthouse results are now recorded in the baseline plan. Codex did not run Lighthouse, build, preview, test, package commands, DB commands, Docker commands, firewall commands, backup/restore commands, route generation, deployment commands, runtime profiling, source changes, or asset optimization.
...
- The section now records the provided page-level Lighthouse scores, evidence provenance, clean preview conditions, and accepted classification.
- The static logo optimization follow-up is reserved for `11G.2b`, not `11G.2a`.

Evidence requirements:

- Human reported `pnpm build` succeeded and `pnpm preview` was used.
- Human reported these results as clean preview conditions compared with prior noisy tests.
- Missing metrics remain marked `not recorded`; no hidden values are guessed.
- Build duration around 15.46s versus earlier 13s builds is treated as normal local variance in this update.

Pending classification:

- Clean browser/no-extension compliance: accepted for operational input based on the human-reported clean preview conditions.
- Asset hygiene: optional P2 only. `/bps-logo.png` is not a blocker from the current clean evidence.
- Cache/compression: remains a deployment-server or final-serving concern unless reproduced in final serving setup.
- TBT/performance: no current blocker. No repeated Performance below 60, repeated TBT above 1000 ms, severe lag, idle request loop, or unbounded growth was reported.
...
Status: evidence recorded. Human-provided manual notes and summarized terminal output were recorded in the dedicated evidence log. Codex did not run backup, restore, database, Docker, firewall, package, build, preview, test, route-generation, deployment, cleanup, migration, seed, or endpoint commands.

Output:

- `docs/migration/phase-11g-backup-restore-evidence.md` now records the 11G.3 human evidence, backup/restore validation, DB/storage alignment summary, residual caveats, and decision classification.
- `docs/migration/phase-11g-backup-restore-runbook.md` now links to the dedicated evidence log and points to 11G.4.
- `docs/migration/phase-11g-operations-plan.md` now marks 11G.3 as evidence recorded.

Evidence classification:

- Backup evidence: PostgreSQL dump created, `pg_restore -l` validation passed, storage backup created with file count 13, manifest created, package metadata reference and non-secret config key names recorded, and backup storage location kept operator-held/redacted.
- Restore evidence: clean local target, target DB clean before restore, PostgreSQL restore completed, storage restore completed with file count 13, app pointed to restored target without printing values, app started in dev mode, login/list/preview/download passed, diagnostics and cleanup dry-run reviewed, and destructive cleanup avoided.
- DB/storage alignment: selected `dokumen_transaksi.lampiran_urls` align, selected archive snapshot handling was reported safe with caveat, referenced files were not listed as orphan in dry-run, and missing referenced files were classified carefully and not auto-cleaned.
...
Rationale: human evidence records successful paired DB/storage backup, successful restore into a clean local target, app validation, diagnostics dry-run review, destructive-cleanup avoidance, and representative DB/storage alignment checks. No blocker was reported.
...
This PASS is bounded to the recorded backup/restore drill. It is not LAN readiness, production readiness, release readiness, go-live approval, or exhaustive full-system certification.

#### Phase 11G.4: LAN Binding And Client Smoke Evidence

Date: 2026-05-21.

Status: bounded LAN smoke evidence recorded as PASS after human retest. A dedicated evidence template was created, a LAN HTTP session-cookie compatibility blocker was reported and fixed in 11G.4a, and a LAN HTTP browser API compatibility blocker was reported and fixed in 11G.4b. Codex did not run LAN binding, firewall, network/interface discovery, app endpoint, package, build, preview, dev-server, broad test, Docker, database, backup/restore, route-generation, deployment, cleanup, migration, or seed commands.

Output:

- `docs/migration/phase-11g-lan-smoke-evidence.md` now records the 11G.4 evidence source, serving/binding summary, client smoke checklist, safety/security checklist, PASS/PARTIAL/FAIL/BLOCKER rules, and human-only evidence checklist.
- `docs/migration/phase-11g-operations-plan.md` now links to the dedicated 11G.4 evidence log and records the bounded PASS plus the next move to 11G.5.

Evidence classification:

- LAN binding evidence: recorded as redacted human summary using `http://<SERVER_LAN_IP>:<APP_PORT>`.
- Firewall scope: not separately recorded in docs.
- PostgreSQL LAN exposure status: no broad PostgreSQL exposure reported.
- Another-device client reachability: LAN app URL reportedly opened successfully.
- LAN client login/session persistence: passed in human retest; `dms_session` and `dms_active_role` were stored, refresh preserved login, `/api/auth/session` authenticated, and logout cleared `dms_session`.
- Role dashboard and document list loading: no blocker reported during authenticated LAN smoke.
- Preview/download from LAN client: valid file preview/download reportedly worked in final retest.
- Ajukan/Revisi additional kelengkapan: passed after 11G.4b; no `crypto.randomUUID is not a function` error remained and duplicate validation still worked.
- Unauthorized/admin denial from LAN client: non-admin `/admin` denial reportedly worked.
- Password-change clearing: not re-recorded in the final 11G.4 smoke summary.
- Cookie/mixed-origin, console, and performance smoke: no functional blocker; insecure HTTP blob warning was observed for download and classified as expected trusted-HTTP limitation, with no idle request loop or unbounded growth observed.

Decision:

```text
PASS for bounded LAN smoke after 11G.4a and 11G.4b human retest
```

Rationale: human evidence identified a real HTTP LAN cookie persistence blocker fixed in 11G.4a and a real HTTP LAN browser API blocker fixed in 11G.4b. Final human retest then confirmed the bounded LAN smoke checks were satisfied. This PASS is limited to trusted LAN smoke and does not imply production readiness, release readiness, operational certification, or go-live approval.

11G.4a fix details:

- `src/lib/auth/session-cookies.ts` centralizes session cookie options for create and clear.
- `DMS_SESSION_COOKIE_SECURE=false` intentionally omits `Secure` for trusted HTTP LAN/local mode.
- `DMS_SESSION_COOKIE_SECURE=true` forces `Secure`.
- Unset/default keeps `Secure` in production or HTTPS/proxy-HTTPS requests.
- `HttpOnly`, `SameSite=Lax`, `Path=/`, session duration, endpoint paths, and response shapes are preserved.
- Logout and password-change clearing use the same option source.
- `dms_active_role` remains UX state only and not authorization proof.

11G.4b fix details:

- `src/lib/utils/client-id.ts` provides `createClientId(...)` for non-security temporary client-side identifiers only.
- Ajukan/Revisi additional kelengkapan rows now use `createClientId('user-custom')` instead of direct browser `crypto.randomUUID()`.
- The helper prefers `globalThis.crypto.randomUUID()`, falls back to `globalThis.crypto.getRandomValues(...)`, and only then uses timestamp plus `Math.random()` for non-security client row identity.
- The fallback must not be used for authentication, authorization, CSRF, password reset, session, persisted entity, or file-access tokens.
...
4. Set `$env:DMS_SESSION_COOKIE_SECURE="false"` for trusted HTTP LAN mode.
...
10. Refresh or navigate and confirm the session persists.
11. Logout and confirm `dms_session` is cleared.
12. Change password and confirm the current session is revoked/cleared.
13. Open Ajukan Dokumen, add additional kelengkapan, and confirm no `crypto.randomUUID is not a function` console error.
14. Submit a valid document if safe, or stop before submit if only testing form behavior.
15. Open Revisi flow if available, add additional kelengkapan, and confirm duplicate validation still works.
16. Confirm preview/download remains working.
17. Confirm non-admin admin access remains denied.
18. Confirm PostgreSQL remains not exposed to LAN.
...
| 11H.3 | Final Release Handoff Classification | Final human classification. | Partial / bounded release handoff is recorded without overclaiming. |
...
- 11H.3 is a bounded governance handoff, not final readiness approval for public production;
