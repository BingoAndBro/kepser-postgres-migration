...
- Internal `/bendahara` route names, endpoint names, and navigation targets remain unchanged.
- Kept PPSPM-only approve/reject controls and visibility gate at `IN_BENDAHARA_APPROVAL`.
...
- PPSPM fetch remains `/bendahara/dokumen/:id`.
- PPSPM approve remains `POST /api/bendahara/dokumen/:id/approve`.
- PPSPM reject remains `POST /api/bendahara/dokumen/:id/reject` with `{ catatan }`.
- PPSPM navigation remains `/bendahara/selesai` and `/bendahara/ditolak`.
- PPSPM detail `Kembali` uses browser history and falls back to `/bendahara/inbox`.
...
- PPSPM detail still passes `apiType="bendahara"` to `AttachmentViewer`.
