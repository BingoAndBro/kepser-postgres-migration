...
Phase 11H.2b update: `docs/migration/phase-11h-p1-security-gate-decision.md` now records that the human selected incremental P1 implementation and that login rate-limit/brute-force protection, destructive admin cleanup hardening, and raw logical-path file-access hardening are implemented pending human retest. Login now has an in-memory local-process limiter with 5 failed attempts per normalized identifier/IP key in 10 minutes and a 15-minute cooldown; this is sufficient as a local single-process foundation but is not restart-persistent or distributed. GET `/api/admin/cleanup-orphan-files` is dry-run/report-only, while destructive cleanup requires POST body `dry_run=false` and `confirm=true`; pending cleanup still additionally requires `include_pending=true` and age eligibility. Raw logical-path preview/download and `/api/files/access` revalidate current document/archive references and block `DIMUSNAHKAN` at token-use time. CSRF/origin strategy remains P1 and is not accepted, downgraded, implemented, or resolved. Next recommendation is `Phase 11H.2a - CSRF/Origin Protection Follow-up` unless a blocker remains in 11H.2b/11H.2c/11H.2d retest, not 11H.3.
...
- Phase 11H.1 is recorded in `docs/migration/phase-11h-final-supabase-audit.md` and Phase 11H.2/11H.2b/11H.2c/11H.2d is recorded in `docs/migration/phase-11h-p1-security-gate-decision.md`. 11H.2b implements login rate-limit/brute-force protection pending human retest, 11H.2c implements destructive admin cleanup hardening pending human retest, and 11H.2d implements raw logical-path file-access hardening pending human retest; CSRF/origin remains pending, no bounded risk is accepted, and no final readiness classification is made. The next recommendation is `Phase 11H.2a - CSRF/Origin Protection Follow-up` unless a blocker remains in 11H.2b, 11H.2c, or 11H.2d retest.
