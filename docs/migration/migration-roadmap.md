...
Phase 11H.2c update: `docs/migration/phase-11h-p1-security-gate-decision.md` now records that the human selected incremental P1 implementation and that destructive admin cleanup hardening is implemented pending human retest. GET `/api/admin/cleanup-orphan-files` is dry-run/report-only, while destructive cleanup requires POST body `dry_run=false` and `confirm=true`; pending cleanup still additionally requires `include_pending=true` and age eligibility. CSRF/origin strategy, login rate-limit/brute-force foundation, and raw logical-path file-access hardening remain P1 and are not accepted, downgraded, implemented, or resolved. Next recommendation is `Phase 11H.2d - Raw Logical-Path File Access Hardening` unless a blocker remains in 11H.2c retest, not 11H.3.
...
- Phase 11H.1 is recorded in `docs/migration/phase-11h-final-supabase-audit.md` and Phase 11H.2/11H.2c is recorded in `docs/migration/phase-11h-p1-security-gate-decision.md`. 11H.2c implements destructive admin cleanup hardening pending human retest; the other P1 decisions remain pending, no bounded risk is accepted, and no final readiness classification is made. The next recommendation is `Phase 11H.2d - Raw Logical-Path File Access Hardening` unless a blocker remains in 11H.2c retest.
