...
- Phase 11F is not complete. `pnpm preview` currently fails at runtime after successful build because `pg` imports unresolved optional `pg-native`, blocking production-like runtime/performance testing. Long-session `pnpm dev` slowdown remains unclassified until preview works.
- Next target: Phase 11F.4a Production Preview Runtime Blocker: pg-native Optional Dependency Resolution. Phase 11G Backup/Restore, LAN Deployment, And Operations Hardening is deferred until preview is fixed and follow-up validation confirms production-like runtime behavior. Final go-live remains human-controlled after regression/smoke evidence and hardening evidence are reviewed.
