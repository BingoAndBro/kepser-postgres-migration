...
Status: evidence pending. A dedicated evidence log was created, but no human backup/restore terminal output, screenshots, manual operator notes, or summarized operator reporting was provided in this phase.

Output:

- `docs/migration/phase-11g-backup-restore-evidence.md` now records the 11G.3 evidence template, required evidence fields, missing validation items, DB/storage alignment checklist, and decision classification.
- `docs/migration/phase-11g-backup-restore-runbook.md` now links to the dedicated evidence log.
- `docs/migration/phase-11g-operations-plan.md` now marks 11G.3 as pending human data.

Evidence classification:

- Backup evidence: pending; PostgreSQL dump, storage backup, manifest, package metadata reference, non-secret config key names, and operator-held backup location are not recorded.
- Restore evidence: pending; clean target, PostgreSQL restore, storage restore, app start, login, document list, preview, download, `DIMUSNAHKAN`, diagnostics, and cleanup dry-run checks are not recorded.
- DB/storage alignment: pending; representative `dokumen_transaksi.lampiran_urls`, `arsip.lampiran_snapshot`, destroyed archive snapshot handling, orphan dry-run, and missing referenced file classification are not recorded.
...
Rationale: missing backup/restore evidence and missing restore validation prevent PASS. No restore failure was provided, so this is not classified as FAIL/BLOCKER beyond the missing-evidence blocker.
...
Do not proceed to 11G.4 until human-run backup/restore evidence is complete enough to classify 11G.3 as PASS.
