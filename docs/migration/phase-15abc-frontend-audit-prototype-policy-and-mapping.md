...
- Lowercase roles such as `user`, `ppspm`, `arsiparis`, `kinerja`
...
- PPK: `src/routes/ppk/**`
- PPSPM internal namespace: `src/routes/ppspm/**`, preserving internal role value `PPSPM`
...
- `PPSPM` internal enum remains unchanged and displays as `PPSPM`.
...
| PPSPM dashboard/inbox/detail | `/ppspm`; `/ppspm/inbox`; `/ppspm/dokumen/$id`; `/ppspm/ditolak`; `/ppspm/selesai` | `src/routes/ppspm/**` | `PPSPM` displayed as `PPSPM` | `/ppspm/inbox`; `/ppspm/dokumen/$id`; approve/reject APIs; PPSPM preview/download APIs | `roles/TreasurerView.tsx`; `workflow/InboxView.tsx`; `workflow/DocumentDetailView.tsx` | approval queue layout, PPSPM action footer, reject modal styling | user-facing `Ppspm` label where PPSPM is required; mock financial approval | `AttachmentViewer apiType="ppspm"`; `ROLE_DISPLAY.PPSPM` | High | 15G | Visual-only rebuild |
...
- `ppspm` route/role references may map to internal `PPSPM`, but user-facing label should be `PPSPM`.
