...
- Generic document signed URL endpoints may serve a storage path only when the path belongs to the current user, or when the current user has a document workflow role (`PPK`, `PPSPM`, `ARSIPARIS`).
...
- Role workspace parent routes (`/ppk`, `/ppspm`, `/arsiparis`, `/admin`) use a lightweight synchronous `beforeLoad` guard in client mode. The guard reads the AppLayout-fed client auth snapshot and must not call Supabase or other async APIs.
