...
- Phase 13Y.1 adds a server-only folder-first physical deletion helper foundation in `src/lib/archive/berkas-arsip-physical-destruction.ts` for berkas that are already `CLOSED/DIMUSNAHKAN`. The helper supports dry-run and exact-confirmation execution with `HAPUS FILE FISIK ARSIP`, derives candidates only from current `berkas_arsip_item` membership plus `dokumen_transaksi.lampiran_urls` or `manual_arsip_attachment.logical_path`, preserves all metadata rows and logical references, returns safe counts/categories only, and is not wired to lifecycle/API/UI yet. Final user-facing integration belongs to the existing `Musnahkan Data` action in a later Phase 13Y.2; the target is not a separate general physical-deletion button or Dimusnahkan list page.
...
- Phase 13O adds read-only folder-first pages and API wrappers for active berkas archives. It points the Kepala Sub Bagian Umum active archive navigation to `/arsiparis/berkas`, keeps old `/arsiparis/aktif` and `/arsiparis/arsip/$id` compatibility pages available, and does not add lifecycle mutation, file access, schema/migration, package/env, storage, or Supabase runtime changes. Phase 13V later changes `/arsiparis/aktif` from an old canonical active list into a compatibility redirect to `/arsiparis/berkas`.
...
- After Phase 13O, the first folder-first active archive pages exist at `/arsiparis/berkas` and `/arsiparis/berkas/$id`. They are read-only surfaces backed by read-only API wrappers and the Phase 13N read model. After Phase 13V, `/arsiparis/berkas` is the primary active archive folder-first surface and `/arsiparis/aktif` is legacy compatibility/redirect only. Old individual archive detail at `/arsiparis/arsip/$id` remains available for historical canonical rows, and old `arsip.arsip` data remains history/compatibility only.
...
- Folder-first physical deletion now has a server-only helper foundation after Phase 13Y.1. It remains dry-run-first, exact-confirmation gated for execution, `CLOSED/DIMUSNAHKAN` only, metadata-preserving, idempotent for missing files, local-storage-only, and limited to candidates derived from current folder items plus source tables. It must never use client-supplied paths, broad storage-root scans, public/static targets, Supabase fallback, or user-facing reports containing logical paths, physical paths, storage roots, tokens, signed-token internals, SQL details, env values, cookies, session values, raw rows, or secrets. Phase 13Y.1 is helper-only; final physical deletion integration belongs to the existing `Musnahkan Data` action in Phase 13Y.2 rather than a separate general UI button.
...
- `/arsiparis/berkas`
- `/arsiparis/berkas/$id`
- `/arsiparis/aktif` legacy compatibility redirect to `/arsiparis/berkas`
- `/arsiparis/inaktif`
