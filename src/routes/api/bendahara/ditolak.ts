...
export const Route = createFileRoute('/api/bendahara/ditolak')({
  server: {
    handlers: {
      GET: async ({ request }: { request: Request }) => {
        ...
        const { data: docs, error } = await supabase
          ...
          .select('id, judul, fungsi_id, kegiatan_jenis_id, tahun, tanggal, created_at, updated_at, revision_notes')
          .eq('status', 'NEED_REVISION').eq('revision_target', 'PPK')
          .order('updated_at', { ascending: false })

        if (error) return Response.json({ error: 'Gagal mengambil data' }, { status: 500 })

        const fungsiIds = [...new Set((docs ?? []).map((d: any) => d.fungsi_id).filter(Boolean))]
        ...
        const kegIds = [...new Set((docs ?? []).map((d: any) => d.kegiatan_jenis_id).filter(Boolean))]
        ...
        return Response.json({ dokumen: (docs ?? []).map((d: any) => ({
          id: d.id, judul: d.judul,
          fungsi_nama: fungsiMap[d.fungsi_id] ?? '—',
          kegiatan_nama: kegMap[d.kegiatan_jenis_id] ?? '—',
          tahun: d.tahun, tanggal: d.tanggal, created_at: d.created_at, updated_at: d.updated_at,
