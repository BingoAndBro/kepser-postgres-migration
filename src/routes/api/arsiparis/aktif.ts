  ...
  server: {
    handlers: {
      GET: async ({ request }: { request: Request }) => {
        ...
        try {
          const result = await getUnifiedArchiveList({ statusArsip: ARCHIVE_STATUS.AKTIF })

          return Response.json({
            aktif: result.rows.map(mapUnifiedArchiveRow),
          })
        } catch {
          console.error('[aktif] local query error')
