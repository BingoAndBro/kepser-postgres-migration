  ...
  server: {
    handlers: {
      GET: async ({ request, params }: { request: Request; params: Record<string, string> }) => {
        ...
        if (!isUuid(params.id)) return Response.json({ error: 'Arsip inaktif tidak ditemukan' }, { status: 404 })
        ...
        try {
          ...
          const row = rows[0]
          if (!row) return Response.json({ error: 'Arsip inaktif tidak ditemukan' }, { status: 404 })
          ...
          console.error('[inaktif/:id] GET local query error:', err)
