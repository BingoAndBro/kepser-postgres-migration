      ...
      <div className="space-y-6">
        <div>
          <div className="flex items-center gap-1.5 text-[10px] font-bold text-outline uppercase tracking-widest mb-2">
            <FileEdit size={12} />
            <Link to="/" className="hover:text-primary">Pegawai</Link>
          ...
          <p className="text-on-surface-variant text-xs mt-1">
            {items.length} dokumen dikembalikan untuk diperbaiki.
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 gap-4 bg-surface-container-low/30 rounded-2xl border border-outline-variant/20">
            ...
            <p className="text-on-surface-variant text-xs">Dokumen yang dikembalikan akan muncul di sini.</p>
          </div>
        ) : (
                  ...
                  {items.map((d, i) => (
                    <TableRow key={d.id} className="group hover:bg-primary/5 transition-colors">
                      ...
                      <TableCell className="text-center">
                        <Link to="/pegawai/dokumen/$id/revisi" params={{ id: d.id }}>
