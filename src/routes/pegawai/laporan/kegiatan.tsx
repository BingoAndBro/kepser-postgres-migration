...
import { BarChart3, ExternalLink, Users, ChevronRight } from 'lucide-react'
...
function LaporanKegiatanPage() {
  ...
  useEffect(() => {
    fetch('/api/laporan/kegiatan', { credentials: 'include' })
      .then(r => r.json())
      .then(d => {
        if (d.error) { setError(d.error); return }
        setDokumen(d.dokumen ?? [])
        setIsKetuaTim(d.isKetuaTim ?? false)
        // Ambil user id dari baris pertama yang is_ketua_tim = true
  ...
  return (
    <PageLayout>
      <div className="space-y-6">
        ...
        {/* State: belum pernah jadi Ketua Tim */}
        {!loading && !error && !isKetuaTim && (
          <div className="flex flex-col items-center justify-center py-24 gap-4 text-center">
            <div className="rounded-full bg-violet-500/10 p-5">
              <Users className="h-10 w-10 text-violet-500/60" />
            </div>
            <div className="space-y-1">
              <p className="text-base font-medium">Anda belum pernah menjadi Ketua Tim</p>
              <p className="text-sm text-muted-foreground max-w-sm">
                Halaman ini akan terisi secara otomatis setiap kali Anda mengajukan dokumen sebagai Ketua Tim.
                Semua dokumen dari Anda dan anggota tim akan terakumulasi di sini.
              </p>
            </div>
          </div>
        )}
        ...
        {!loading && !error && isKetuaTim && filtered.length === 0 && dokumen.length > 0 && (
        ...
        {/* Tabel */}
        {!loading && !error && isKetuaTim && filtered.length > 0 && (
          <>
            {/* Ringkasan */}
