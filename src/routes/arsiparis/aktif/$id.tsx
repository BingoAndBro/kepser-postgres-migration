...
import {
  ...
  Eye, X, ArrowRightCircle,
} from 'lucide-react'
import type { LampiranUrl } from '#/lib/dokumen-helpers'
import { cn } from '#/lib/utils'

export const Route = createFileRoute('/arsiparis/aktif/$id')({ component: ArsipAktifDetailPage })
...
function ArsipAktifDetailPage() {
  ...
  async function fetchData() {
    ...
    try {
      const res = await fetch('/api/arsiparis/aktif/' + id, { credentials: 'include' })
      if (!res.ok) {
  ...
  async function handlePindahkan() {
    ...
    try {
      const res = await fetch('/api/arsiparis/aktif/' + id + '/pindahkan', {
        method: 'POST',
        credentials: 'include',
      ...
      window.location.href = '/arsiparis/inaktif'
    ...
    <PageLayout>
      <div className="max-w-3xl mx-auto space-y-6">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center" onClick={e => { if (e.target === e.currentTarget) { setConfirmOpen(false); setCatatan(""); setActionError(null) } }}>
            ...
            <div className="relative z-10 w-full max-w-md mx-4 bg-white rounded-2xl shadow-2xl">
              <div className="flex items-center gap-3 px-5 py-4 border-b border-outline-variant/30">
                ...
                <p className="font-semibold text-on-surface">Pindahkan ke Inaktif?</p>
              </div>
              <div className="p-5 space-y-4">
                ...
                <div className="flex gap-3">
                  ...
                  <Button className="flex-1 gap-1.5" onClick={handlePindahkan} disabled={!!actionLoading}>
                    {actionLoading ? <Loader2 size={14} className="animate-spin" /> : <ArrowRightCircle size={14} />}
                    Ya, Pindahkan
                  </Button>
                </div>
              </div>
        ...
        <div className="flex items-center gap-1.5 text-[10px] font-bold text-outline uppercase tracking-widest">
          ...
          <Link to="/arsiparis/aktif" className="hover:text-primary">Arsip Aktif</Link>
          <ChevronRight size={10} />
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <Link to="/arsiparis/aktif"><Button variant="outline" size="sm" className="mt-4">Kembali</Button></Link>
          </div>
        ) : (
          <>
            ...
            <div className="bg-white rounded-xl border border-outline-variant/30 p-5 shadow-sm">
              ...
              <Button className="w-full gap-1.5" onClick={() => setConfirmOpen(true)}>
                <ArrowRightCircle size={14} />Pindahkan ke Inaktif
              </Button>
            </div>

            <Link to="/arsiparis/aktif">
