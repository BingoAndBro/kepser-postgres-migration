...
function ArsiparisDashboard() {
  const [stats, setStats] = useState<Stats>({ inbox: 0, aktif: 0, verifikasi: 0, inaktif: 0, usulMusnah: 0 })
  ...
  useEffect(() => {
    async function fetchStats() {
      setLoading(true)
      try {
        const [inboxRes, aktifRes, verifikasiRes, inaktifRes, musnahRes] = await Promise.all([
          fetch('/api/arsiparis/inbox', { credentials: 'include' }),
          fetch('/api/arsiparis/aktif', { credentials: 'include' }),
          fetch('/api/arsiparis/verifikasi-penyusutan', { credentials: 'include' }),
          fetch('/api/arsiparis/inaktif', { credentials: 'include' }),
          fetch('/api/arsiparis/usul-musnah', { credentials: 'include' }),
        ])
        const [inboxJson, aktifJson, verifikasiJson, inaktifJson, musnahJson] = await Promise.all([
          inboxRes.json().catch(() => ({ inbox: [] })),
          aktifRes.json().catch(() => ({ aktif: [] })),
          verifikasiRes.json().catch(() => ({ verifikasi: [] })),
        ...
        setStats({
          ...
          verifikasi: (verifikasiJson.verifikasi ?? []).length,
  ...
  const statCards = [
    ...
    { label: 'Verifikasi Penyusutan', value: stats.verifikasi, icon: Archive, color: 'text-amber-500', key: 'verifikasi' },
