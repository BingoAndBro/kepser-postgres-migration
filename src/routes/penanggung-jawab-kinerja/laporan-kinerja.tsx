      ...
      <div className="mx-auto w-full max-w-[1280px] space-y-7 px-7 pt-6 sm:px-8 lg:px-10">
        ...
        {!loading && !forbidden && !error && dokumen.length > 0 && !selectedFungsi && (
          <>
            ...
            <SimpleReportToolbar
              ...
              searchLabel="Cari fungsi Laporan Kinerja"
            ...
            {filteredFungsiRows.length === 0 ? (
              ...
              <FungsiList rows={filteredFungsiRows} onSelect={(id) => selectFungsi(id, navigate)} />
            )}
          </>
        )}
        ...
        {!loading && !forbidden && !error && selectedFungsi && !selectedKegiatan && (
          <FungsiDetailView
            ...
            sortBy={sortBy}
            onSortChange={setSortBy}
            onBack={() => selectFungsi(null, navigate)}
            onSelectKegiatan={(id) => selectKegiatan(selectedFungsi.id, id, navigate)}
          />
        )}
        ...
        {!loading && !forbidden && !error && selectedFungsi && selectedKegiatan && (
          <KegiatanDocumentView
            ...
            onBack={() => selectKegiatan(selectedFungsi.id, null, navigate)}
      ...
      <div className="flex min-w-0 items-start gap-5">
        ...
        <div className="min-w-0">
          ...
          <p className="mt-1 max-w-2xl text-sm font-medium leading-6 text-zinc-700">
            Pantau dokumen final berdasarkan fungsi dan kegiatan.
          </p>
        </div>
      </div>
  ...
  return (
    <div className="overflow-hidden rounded-[26px] border border-zinc-200/80 bg-[#FFFDF9] shadow-[0_3px_14px_rgba(15,23,42,0.07)]">
      <div className="flex flex-col gap-3 border-b border-zinc-100 p-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <label>
          <span className="sr-only">Urutkan Laporan Kinerja</span>
...
function FungsiDetailView({
  ...
  return (
    <>
      <ReportBackHeader
        title={fungsi.nama}
        subtitle="Laporan Kinerja / Fungsi"
      ...
      <SimpleReportToolbar
        ...
        searchLabel="Cari kegiatan Laporan Kinerja"
...
function selectFungsi(id: string | null, navigate: ReturnType<typeof useNavigate>) {
  navigate({
...
function selectKegiatan(fungsiId: string, kegiatanId: string | null, navigate: ReturnType<typeof useNavigate>) {
  navigate({
    to: '/penanggung-jawab-kinerja/laporan-kinerja',
    search: kegiatanId ? { fungsiId, kegiatanId } : { fungsiId },
  })
}
